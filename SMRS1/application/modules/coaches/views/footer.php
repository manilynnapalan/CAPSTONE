...
<script src="<?= base_url('assets/adminlte/')?>plugins/fullcalendar/main.js"></script>
  ...
  $('.update_athletes').on('click',function(){
    ...
    $('input[name="up_year_enrolled"]').val(data['year_enrolled']);
    $('input[name="up_parent_number"]').val(data['parent_number']);
    $('select[name="up_gender"]').append('<option selected="selected" class="added">'+gender+'</option>');
    $('select[name="up_school_year"]').append('<option selected="selected" class="added">'+data['school_year']+'</option>');
  })
