...
class Coaches extends MX_Controller {
	...
	public function update_athlete(){
		...
		$contact_number = $this->input->post('up_contact_number');
		$parent = $this->input->post('up_parent');
		$parent_number = $this->input->post('up_parent_number');
	...
	public function athletes_attendances(){
		...
		foreach($allAthletes as $row){
			$where2 = ['coach_id' => $row1->account_id ];
	...
	public function present_athletes(){
		...
		if($this->Model->getScuaaGamesByRow('surveys_to_answer',$data1) == NULL){
			$this->Model->insertData('surveys_to_answer',$data1);
		}
	...
	function checkAccountNotNull(){
		...
		if($id == NULL){
			...
			if($rows->usertype != 2){
				$message = base64_encode("errorrr~Restricted page. Your account is not a coach type.");
				redirect(base_url('?m='.$message));
			} else {
				return $rows;
			}
