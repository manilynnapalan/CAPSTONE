...
class Admin extends MX_Controller {
	...
	public function home(){
		...
		$allColors = $this->Adm_model->getAllData('colors');
		// var_dump($data['allColors']);
	...
	public function insert_athletes(){	
		...
		if($checkUN == NULL){
			...
			$year_enrolled = $this->input->post('year_enrolled');
	...
	public function update_athlete(){
		...
		$year_enrolled = $this->input->post('up_year_enrolled');
	...
	public function medical_staff(){
		$row = $this->checkAccountNotNull();
		$where = ['usertype'=>4];
		$data['allMedical'] = $this->Adm_model->getAllRowsByUsertype($where);
		$this->heading($row);
		$this->load->view('medical_staff',$data);
		$this->load->view('footer');
	}
	...
	public function insert_staff(){	
		...
		$validty = $this->input->post('validty');
		$username = $this->input->post('username');
		$where = array(
			'username' => $username
		);
      ...
      $image_path = './assets/pro_pic_images/'.$image_name;
        ...
        $accounts_data = [
		    ...
		    $info_data = [
				...
				$check = $this->Adm_model->insertData('information',$info_data);
				if($check != NULL){
        	$message = base64_encode('success~New medical staff successfully added.');
				} else {
					$message = base64_encode("errorrr~There's an error in saving the data. Please contact the Developer.");
				}	
    } else {
   	...
   	redirect(base_url('admin/medical_staff/?m='.$message));
	}
	...
	public function update_staff(){
		...
		$message =  base64_encode("success~Medical staff information successfully updated.");
		redirect(base_url('admin/medical_staff/?m='.$message));
	}
	...
	public function checkAttendance(){
		...
		$no_presents = 0;
		foreach ($data['AllAthletes'] as $row2) {
			if(in_array($row2->account_id, $array_athletes_id)){
				$no_presents++;
			}
		}
